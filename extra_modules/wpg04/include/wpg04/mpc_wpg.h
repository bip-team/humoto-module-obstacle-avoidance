    ...
    namespace wpg04
    {
        ...
        class HUMOTO_LOCAL MPCforWPG : public humoto::MPC
        {
            ...
            public:
                  ...
                  // In case of multiple obstacles, we have to apply to each of
                  // them a collision avoidance constraints.
                  // The CoM condense vector must be duplicated in a vector.

                  // Eigen::MatrixXd         CoM_position_duplicator;
            ...
            private:
                ...
                // This is where the model and the state machine are updated at
                // each iteration.
                humoto::wpg04::ModelState   initializeNextModelState(
                        ...
                        humoto::wpg04::Logger                           &logger) const
                {
                    ...
                    // This is the update of the linear system
                    // c+ = Ac + Bz
                    // evaluate update a "PointMassState" object and return the
                    // a "com_state" (updated).
                    // The com_state_ is a 9-Dim vector,
                    // But the model of S_,s_ is in 2dim
                    ...
                    // From here, let s try to use the logger of Stanislass that
                    // generate a python script to plot the resutls.
                    // We have to understand which input is possible to give.

                    // I added the function "getStateVector()" to export in a
                    // 9Dim vector the entire com_state_
                    newState = model_state.getStateVector();

                    // Added to use phyton and print the results.
                    logger.addStateAndControl(newState,
                                              com_height,
                                              humoto::g_gravitational_acceleration,
                                              model.obstacles_);
            ...
            public:
                ...
                // logger_(mpc_parameters.getSubsamplingTime())
                            ...
                            const humoto::wpg04::WalkParameters             &walk_parameters)
                {
                    ...
                    {
                        // decision variables
                        // It only initialize a dimension: create a structure
                ...
                humoto::wpg04::ModelState   getNextModelState(
                        ...
                        const humoto::walking::StanceFiniteStateMachine &stance_fsm,
                        const humoto::wpg04::Model                      &model,
                        humoto::wpg04::Logger                     &logger)
                {
                    HUMOTO_ASSERT(solution_is_parsed_ == false, "The solution is parsed.");
                    parseSolution(solution);
                    return(initializeNextModelState(stance_fsm, model,logger));
                }
                ...
                //  It computes the current CoM state.
